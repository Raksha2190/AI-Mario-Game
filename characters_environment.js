...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    //Replace press any arrow with play button
...
function manualControl(character){
  ...
  if(character.live){
    //Replace if(keyDown(control.left)) to if(noseX < 300)
    if(noseX < 300){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }
  //Replace if(keyDown(control.right)) to if(noseX < 300)
    if(noseX > 300){
        character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
...
function jumping(character){
    //Replace if(keyWentDown(control.up)) to if(noseY < 168)
